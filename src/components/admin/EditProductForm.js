...
const EditProductForm = ({
  ...
  const initialValues = {
    name: currentProduct?.name || "", // Cung cấp giá trị mặc định
    price: currentProduct?.price || 0, // Cung cấp giá trị mặc định
    cate: currentProduct?.cate?._id || "", // Cung cấp giá trị mặc định
    color: currentProduct?.color || [], // Cung cấp giá trị mặc định
    images: [], // Chỉ hiển thị tên file, không lưu đường dẫn khi chỉnh sửa
    offer: currentProduct?.offer || 0, // Cung cấp giá trị mặc định
    configDesc: currentProduct?.configDesc
      ? Object.entries(currentProduct.configDesc).map(([key, value]) => ({
          key,
          value,
        }))
      : [], // Cung cấp giá trị mặc định
    description: currentProduct?.description || [], // Cung cấp giá trị mặc định
  };
